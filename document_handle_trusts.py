...
#slice file separately to law and principles
#database=database[(database["source"].str.contains("信託法規"))]
#database=database[(database["source"].str.contains("信託實務"))]
...
for i in range(database.shape[0]):
    ...
    conditions  = [ database["answers"][i]=="1", database["answers"][i]=="2", database["answers"][i]=="3",database["answers"][i]=="4" ]
...
file_path = "trusts_exam_print_short.txt"
...
file_path = "trusts_exam_print_long.txt"
with open(file_path, "w") as o:
    with contextlib.redirect_stdout(o):
        for i in database.index:
